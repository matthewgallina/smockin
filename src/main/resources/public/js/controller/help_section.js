...
app.controller('helpSlidesController', function($scope, $uibModalInstance, data) {
    ...
    if (data.name == 'rule_response') {
        ...
        addSlide(seqDir + "/add_seq.png", "Fill in the status code and body for this 1st response, then click 'Add'");
        addSlide(seqDir + "/final_endpoint.png", "Repeat twice more to give 3 responses in total. Once done review and click 'Save'");
        ...
        addSlide(proxyDir + "/curl1.png", "Call the /wait endpoint. This will result in the terminal waiting on a response");
        addSlide(proxyDir + "/curl_timeout.png", "The call will internally timeout after 30 seconds resulting with a 404");
        addSlide(proxyDir + "/curl3.png", "Call the /wait endpoint again. This time post a response from a separate client (see below window)");
        addSlide(proxyDir + "/curl4.png", "The response posted is received by the waiting client (in top window)");
